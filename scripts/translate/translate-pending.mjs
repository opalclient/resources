...
// --- nested-JSON catalogs (web, launcher) ---
// MT input is ALWAYS the en value: the locale file may hold an en copy (same
// thing) or a bad earlier translation (must not be re-translated from). Both
// the marketing site (`web/`) and the launcher UI (`launcher/`) share this
// exact shape — a per-locale nested JSON mirrored against `en.json`.
async function translateJsonCatalog(section) {
  const pending = manifest[section] ?? {};
  if (Object.keys(pending).length === 0) return; // nothing queued; don't touch the dir
  const enData = JSON.parse(readFileSync(join(ROOT, section, 'en.json'), 'utf8'));
  for (const [locale, keys] of Object.entries(pending)) {
    if (only && !only.includes(locale)) continue;
    const file = join(ROOT, section, `${locale}.json`);
    const data = JSON.parse(readFileSync(file, 'utf8'));
    const remaining = [];
    for (const key of keys) {
      const value = getPath(enData, key);
      if (value === undefined) continue; // key vanished; drop from manifest
      try {
        if (!dry) setPath(data, key, await translateValue(value, locale));
        done++;
      } catch (e) {
        console.error(`${section}/${locale} ${key}: ${e.message}`);
        remaining.push(key);
        failed++;
      }
    ...
    if (!dry) {
      ...
      if (remaining.length > 0) manifest[section][locale] = remaining;
      else delete manifest[section][locale];
    }
    console.log(`${section}/${locale}: ${keys.length - remaining.length}/${keys.length} translated`);
  }
}

await translateJsonCatalog('web');
await translateJsonCatalog('launcher');
